...
def lmdb_load(lmdb_obj, frame_file):
    tmpdir = os.getenv("SLURM_TMPDIR")
    with lmdb_obj.begin() as txn:
        lmdb_key = frame_file.replace(tmpdir+'/', "").replace("/train", "").replace("/test", "")
        meta_key = f"meta_{lmdb_key}".encode("utf-8")
        value = txn.get(lmdb_key.encode("utf-8"))
        meta = eval(txn.get(meta_key).decode("utf-8"))
        img = np.frombuffer(value, dtype=np.dtype(meta["dtype"])).reshape((224,224,3))
        return img
    
def load_frame(frame_file, resize=False, lmdb_obj=None):
...
def load_rgb_batch(frames_dir, rgb_files, frame_indices, resize=False, lmdb_loading=False):
    ...
    for i in range(frame_indices.shape[0]):
            ...
            if isinstance(rgb_files[0], str):
                frame_file = os.path.join(frames_dir, rgb_files[frame_indices[i]])
            else:
                frame_file = rgb_files[frame_indices[i]]
...
class VideoDataset_EpicKitchens(Dataset):
    ...
    def __init__(self, csv_file, frequency = 4, num_nodes = 16, is_test = False, transform = None, base_dir='./data/epic_kitchens'):
        ...
        self.file_lookup = dict(pkl.load(open("d1d2.pkl", "rb")))
    def __len__(self):
        return len(self.uid)
